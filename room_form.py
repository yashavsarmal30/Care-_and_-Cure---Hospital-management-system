...
class Room:
    ...
    def INSERT_ROOM(self):
        ...
        if x != 0:
            ...
            tkinter.messagebox.showerror("HOSPITAL DATABASE SYSTEM", "ROOM ALLOCATED")
            self.clear()
            conn.commit()
...
class S_Room:
    ...
    def ROOM_DISPLAY(self):
        ...
        c1.execute('SELECT * FROM ROOM WHERE PATIENT_ID = %s', (pat_rm,))
        p = c1.fetchall()
        if len(p) == 0:
            tkinter.messagebox.showerror("HOSPITAL DATABASE SYSTEM", "PATIENT NOT ALLOCATED ROOM")
        else:
            c1.execute('SELECT PATIENT_ID, NAME, ROOM_NO, ROOM_TYPE FROM ROOM NATURAL JOIN PATIENT WHERE PATIENT_ID = %s',(pat_rm,))
            t = c1.fetchone()
